...
import {
	setCurrentDrink,
	setCurrentDrinkLoading,
	setCurrentDrinkError,
} from "../slices/currentDrinkSlice";

const Drink = () => {
	const { strDrink } = useParams();
	const { data, isLoading, isSuccess, isError } =
		useGetCurrentDrinkQuery(strDrink);
	const dispatch = useDispatch();

	if (isSuccess && data && data.drinks && data.drinks[0])
		dispatch(setCurrentDrink(data.drinks[0]));
				...
				<>
					{data.drinks[0] ? (
						<div
							...
							className="card meal flex-grow-1 m-2"
						>
							<img
